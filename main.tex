...
In this framework, the learning occurs through the enaction of control loops that implement Piagetian \textit{sensorimotor schemes} \cite{piaget2013construction} we call \textit{interactions}.
...
1) learning to define, recognize and localize other agents that make random movements in the environment,
...
As a PRI agent can only perceive its environment through enacted interactions, we define the signature $S_i$ of an interaction i as an emerging structure characterizing one (or more) set(s) of interactions $\{j_k\}$ whose enaction (i.e. $\{j_k\} \in E_{t}$ ) can characterize the presence of an element affording $i$ for next step $t+1$.

Defining objects by learning the affordances they provide is abundant in literature
