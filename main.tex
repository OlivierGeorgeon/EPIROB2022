...
At the end of step $t$, coinciding with the beginning of step $t+1$, the agent receives, not one, but a set of $k$ enacted interactions $E_{t+1}=\{e_{1,t},..., e_{k,t}\} \subset I$. 
...
If the experimental settings satisfy the Markov property, we can use the set $E_t \subset I$ of the interactions enacted during step $t-1$ as the context upon which the agent estimates the probability of success of interactions during step $t$. Otherwise, we must chain sequences of interactions to extend the temporal dependency. 
...
$S_i(E_t) \in [-1, 1]$ gives the estimated possibility of successfully enacting interaction $i$ at step $t$: 1 for certainty of success, -1 for certainty of failure. 
To improve this estimation, the agent adjusts $S_i(E)$ each time $i$ succeeds or fails in context $E$. %Previous implementations of signatures used formal neurons [ ] or discrete ensembles of interactions [ ].

Our signatures of interaction are \textit{reversible} : they allow defining a reverse function $\hat{S}_i : \{-1;1\} \rightarrow \mathcal{P}(\mathcal{P}(I))$ such that $\hat{S}_i(-1)$ gives the set of minimal contexts in which the enaction of $i$ is impossible, and $\hat{S}_i(1)$ gives the set of minimal contexts in which it is possible, i.e., the contexts that \textit{afford} $i$. 
...
However, when interacting with mobile entities, the presence of an affordance at the end of step $t-1$ does not guaranty that the interaction can be enacted during step $t$ because the entity may move in the meantime.
To address this issue, this study separates the estimation of the affordance from the estimation of the probability of success.
...
\subsection{Separating affordances from probability of success}\label{AA}

From the observer's perspective, three types of situations can happen when the agent tries an interaction withe a mobile entity:
\begin{enumerate}
...
The existence of situations of type 2 distorts the learning of signatures, causing the prediction $S_i(E)$ to remain negative even though the affordance is present.
%, and thus the possibility to distinguish situations 1 and 2 from situations 3.
We nonetheless observed that, although negative, the prediction remained slightly higher than when the affordance was absent due to the existence of situations of type 1. 
%influence the signature, allowing the context of interactions corresponding to the affordance to emerge while remaining insufficient to predict with a positive value.
...
Symmetrically, some interactions may fail due to the unexpected apparition of an entity in a specific location (e.g., trying to move forward while an obstacle appears in front). 
In such situations, we do not decrease the prediction of success if it is lower than the threshold of the average of positive predictions (i.e., $S_i(E_t)<\overline{S_i^s}$ if $\overline{S_i^s} > 0.9$).  

%Several interactions can be prevented by the presence of an entity (e.g., \textit{moving a step forward} is expected to succeed, until an obstacle appears). In such cases, situations 1' are related to the failure of the interaction, and situations 2' and 3' to its success.  As situations 2' and 3' are expected to be more frequent than situations 1', the average of predictions will converge to a positive value.  We thus used the average of positive predictions $\overline{S_i^s}$ as a threshold to prevent the signature reinforcement in case of success in an assumed situation 2' (i.e. $S_i(E_t)<\overline{S_i^s}$).  This principle is applied when $\overline{S_i^s}$ is greater than a threshold (we used a threshold of 0.9).
...
It is also possible to define the ratio of success when the prediction $S(E_t)$ is greater than the average $\overline{S^f}$ (or failure when $S(E_t)$ is lower than the average $\overline{S^s}$), implying that the agent is in a situation of type 1 or 2.
% (or 1' or 2'). 
This ratio thus measures the probability that the interaction will succeed in the presence of the affordance by a mobile entity.
...
This signature mechanism was tested on an artificial agent moving in the 2-dimensional discrete environment shown in Figure~\ref{fig:environment}. 
...
The environment contains three types of entities offering spatial regularities that the agent can discover by interacting with them, and characterized by a color that makes them recognizable through agent's visual system: 1) wall (green), affording bump, 2) algae (red), that are walkthroughable (and thus useless in the agent's perspective), and 3) fishes (blue), affording eat.
...
\caption{Test environment. The agent is represented as a grey shark (bottom left), wall as green blocks, algae as red leafs and mobile preys as blue fish. 
At each simulation step, each fish has 20\% chance of remaining at its current position or to move up, down, left or right. 
%This behavior simulates an agent with an unknown decisional mechanism.
}
