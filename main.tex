...
From the observer's perspective, three types of situations can happen when the agent tries an interaction:
\begin{enumerate}
...
The existence of situations of type 2 distorts the learning of signatures, causing the prediction $S_i(E)$ to remain negative even though the affordance is effectively present.
...
Situations in which such interactions fail do not decrease the prediction of success if it is lower than the threshold of the average of positive predictions (i.e. $S_i(E_t)<\overline{S_i^s}$ if $\overline{S_i^s} > 0.9$).  
...
It is also possible to define the ratio of interaction success when the prediction $S(E_t)$ is greater than the average $\overline{S^f}$ (or interaction failure when $S(E_t)$ is lower than the average $\overline{S^s}$), implying that the agent is in a situation of type 1 or 2.
% (or 1' or 2'). 
This ratio thus measures the probability that the presence of the affordance actually affords successfully the interaction.
...
As we only study the emergence of signatures, we define a unique learning mechanism that foster interactions with low certitude of success or failure (low $|S_i(E_t)|$). %Note that a secondary interaction can be candidate if its associated primary interaction is predicted as a success with a high certitude.
