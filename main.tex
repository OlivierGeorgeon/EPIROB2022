...
To help discovering spatial regularities of the environment, we have proposed the Parallel-RI (PRI) model which allows the simultaneous enaction of multiple interactions. At the end of step $t$, the agent receives a set $E_t=\{e_1, ..., e_k\}_t \subset I$ of $k$ enacted interactions instead of a single enacted interaction. 
...
The agent learned to localize distant affordances (possibilities of enacting an interaction), and to store and keep track of them in an emergent structure that played the role of spatial memory. 
