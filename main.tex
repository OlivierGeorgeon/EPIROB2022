...
Allowing robots to learn by themselves to coordinate their actions or to cooperate requires that they be able to recognize each other and be capable of intersubjectivity.
To comply with artificial developmental learning and self motivation, we follow the \textit{radical interactionism} hypothesis, in which an agent has no \textit{a priori} knowledge on its environment nor its spatial structure, and does not receive rewards defined as direct function of environment states.
...
In this paper, we propose new mechanisms allowing to identify and localize such mobile entities. At this end, an agent has to learn the relation between its perception of mobile objects and the interactions that they afford. These relations are recorded under the form of data structures, called signatures of interaction, that characterize objects in its point of view, and whose properties are exploited to detect distant objects. %The position of a mobile object is defined here by its most probable predicted positions.
...
These mechanisms were tested in a simulated prey-predator environment. The obtained signatures showed that the agent (predator) successfully learned to identify mobile preys and their probabilistic properties, and can detect distant preys in its surrounding environment. %, without using \textit{a priori} knwoledge on spatial structure of the environment.
\end{abstract}
