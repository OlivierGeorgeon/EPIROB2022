...
This study is situated within the framework of artificial constructivist learning, e.g., \cite{wang_new_2012} and enactive learning, e.g., \cite{froese2009enactiveAI}. 
...
This view on self motivation relates to the notion of \textit{intrinsic motivation} of artificial agents for \textit{developmental learning}, e.g., \cite{oudeyer_intrinsic_2007}.
...
we pursue the long-term goal of allowing the emergence of social interactions within groups of artificial agents, e.g., \cite{nsfkatz10}. Generating such collective behaviors %involving other agents
...
In contrast with most machine learning approaches, an RI agent cannot directly detect states of its environment: %through perceptual data.
...
Previous PRI experiments showed that the agent was able to define, recognize and localize affordances \cite{gibson:affordances}), %of static entities through the visual interactions that they afforded (relative displacement of salient points).
...
To improve this estimation, the agent adjusts $S_i$ each time $i$ succeeds or fails. %Previous implementations of signatures used formal neurons [ ] or discrete ensembles of interactions [ ].
